...
import { LayoutDashboard, Users, Settings, ChevronRight, ChevronLeft, BookOpen, UserSquare, TrendingUp, GraduationCap } from 'lucide-react';
        ...
        <aside className={`bg-gray-800 text-gray-300 transition-all duration-300 ease-in-out z-30 fixed lg:relative inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${isSidebarExpanded ? 'w-64' : 'w-20'}`}>
            ...
            <nav className="p-2">
                <ul>
                    ...
                    <NavItem to="/alumnos" icon={GraduationCap} text="Alumnos" />
                    <NavItem to="/cursos" icon={BookOpen} text="Cursos" />
                    <NavItem to="/docentes" icon={Users} text="Docentes" />
                    <NavItem to="/predictivos" icon={TrendingUp} text="Predictivos" />
                    <hr className="my-2 border-gray-700"/>
                    <NavItem to="/users" icon={UserSquare} text="Admin Usuarios" />
